...
sys.path.append("../")
from benchmarking import c_SHAP, LRP_score, PBFI
from benchmarking.neuralized_kmeans import *
from clim import ClusterLOCOMP, ClusterLOCO_RAMPART
from clim.utils import match_labels, hinge_error
...
def main():
    ...
    g = ClusterLOCOMP(base_clusterer=model, K=K, B=B)
    g.fit(X, patch_n=patch_n, patch_m=patch_m, parallel_MP=True, parallel=par, standardize=False)
    
    locomp_raw = g.score(hinge_error, z = g.z_ref, agg='by_clusters',parallel_features=False) # make sure that there are K clusters via z_ref compared to z_test
    ...
    gen_fn = ClusterLOCO_RAMPART(base_clusterer=model, K=K, error_metric=hinge_error, 
                                 parallel_MP=True, parallel=par, standardize=False, alpha_N = alpha_N, alpha_M = alpha_M)
    out = RAMPART(X, generalizability_fn=gen_fn, B=B_ramp, ranking_fn=transform_scores_to_ranking, top_k=topk, gen_kwargs={})

    rampart_info = out['final_model'].score(hinge_error, agg='by_clusters',parallel_features=False)
    res_dict['rampart']={}
