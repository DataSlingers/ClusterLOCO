"""
Cluster LOCO-RAMPART for adaptive feature trimming of Cluster LOCOMP

Author: Claire He
Cluster LOCO-RAMPART code is inspired by the RAMPART Chen (TMLR 2025)
...
def RAMPART(X, generalizability_fn, B, ranking_fn, B_schedule=True, alpha_N=0.25, alpha_M=0.1, top_k=None, 
            rampart_verbose=True, save=False, gen_kwargs=None,):
    """
    ...
    --------
    X: ndarray, full data
    generalizability_fn: callable, generalizability function 
    ...
    for t in range(1, T + 1):
        ...
        res_t, _ = generalizability_fn(X_sub, B=B_t, alpha_N=alpha_N, alpha_M=alpha_M, feature_list=index_set.copy(), **gen_kwargs)
        phi_t, std_t, _ = _extract_phi_std(res_t, M=len(index_set)) # phi_t is local to X_sub, length len(index_set)
        tau_local, _ = ranking_fn(phi_t) # Local ranking among current features
        tau_global = index_set[tau_local] # Map local ranking back to global feature ids.
        ...
        if len(index_set) <= top_k:
            break
    X_final = X[:, index_set] # Final evaluation on surviving set.
    res_final, g = generalizability_fn(X_final, B=B, alpha_N=alpha_N, alpha_M=alpha_M, feature_list=index_set.copy(), **gen_kwargs)
    ...
    selected_std = std_final[tau_local_final[:k]] if std_final is not None else None
...
def ClusterLOCO_RAMPART(*, K: int, error_metric=None, proba_error: bool = True, base_clusterer=None, base_classifier=None,
    random_state: int = 0, parallel_MP: bool = True, parallel: dict | None = None, alpha_N=0.1, alpha_M=0.4,
    standardize: bool = False, reference: str = "full", agg: str = "mean"):
    """
    ...
    def generalizability_fn(X_sub, *, B, alpha_N=alpha_N, alpha_M=alpha_M, **_ignored,):
        X_sub = np.asarray(X_sub)

        model = ClusterLOCOMP(K=K, B=B, base_clusterer=base_clusterer, base_classifier=base_classifier, random_state=random_state)
        model.fit(X_sub, y=None, alpha_N=alpha_N, alpha_M=alpha_M, parallel_MP=parallel_MP, parallel=parallel, standardize=standardize, reference=reference)
        z = model.z_ref

        sc = model.score(error_metric=error_metric, z=z, agg=agg, features=None, proba_error=proba_error, parallel_features=False, par={"n_jobs": 1, "backend": "loky", "prefer": "processes", "verbose": 0})
        # Return in RAMPART's old expected shape: phi/std aligned to X_sub columns.
        M_sub = X_sub.shape[1]

        phi = np.full(M_sub, np.nan, dtype=float)
        std = np.full(M_sub, np.nan, dtype=float) if sc.get("delta_se", None) is not None else None
...
def _extract_phi_std(res, M: int | None = None):
    """
    RAMPART runs on the old version of Cluster LOCOMP
